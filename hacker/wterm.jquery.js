(function ($) {
    ...
    var get_defaults = function () {
        ...
        return {
            ...
            NOT_FOUND: '<p>Unknown command</div>',
    ...
    function time() {
        if (gameNumber == 0) {
            document.getElementById('countdown').innerHTML = demo_game_seconds;
            demo_game_seconds--;
            if (demo_game_seconds == 0) {
                window.location = "score";
            }
        }
        else {
            document.getElementById('countdown').innerHTML = real_game_seconds;
            real_game_seconds--;
            if (real_game_seconds == 0) {
                window.location = "score";
            }
        }
    }
    ...
    function sendCommand(command,data) {
        ...
        $.ajax({
            type: 'POST',
            url: 'input_command',
            data: "input=" + command + "&output=" + data,
            success: function(data){
    ...
    $.fn.wterm = function (options) {
        ...
        return this.each(function () {
            ...
            if (settings.WIDTH && settings.HEIGHT) element.css({
                width: settings.WIDTH,
                height: settings.HEIGHT
            })

            element.html('').append('<br/>' + '<div>' + settings.WELCOME_MESSAGE +  'Probe the network -> Attack the System -> Steal the information<br><br></div>');

            //Probe Phase<br>-----------------<br>Probing means that you try to collect information on whether a webserver is a honeypot or a regular webserver.<br>To probe a webserver, you need to run the nmap command on each webserver once.<br>In response to your action, the network would reply with the list of open ports, the services running on them along with their vulnerabilities on the corresponding webservers.<br>Example: nmap system1<br><br>' + '</div>')

            element.append('<div class="' + settings.CONTENT_CLASS + '"></div>');
            element.append('<div><span class="' + settings.PROMPT_CLASS + '">' + '>> <span id="why"> </span> ' + 'Remaining Time  : ' + '<span id="countdown">  </span> seconds'+
            '<span id="res"></span>'+ '&nbsp;'+

            '<table><tr><td style="background-color: #845946">></td><td width="100%"><form> <input type="text" ></form></td></tr></table></div>');



            // Representing prom  pt, form, input and content section
            // in the terminal
